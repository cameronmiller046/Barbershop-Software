...
import { LINK_APP, LINK_CAPABILITIES, sha256Hex, verifyLink, yggdrasilEnabled } from "@/lib/yggdrasilLink";

export const dynamic = "force-dynamic";
export const runtime = "nodejs"; // needs node:crypto for hashing

// POST /api/yggdrasil/pair — pairing with the Yggdrasil management plane.
...
//
// Bootstrap protection: when YGGDRASIL_LINK_SECRET is configured, even the FIRST
// pairing must authenticate with it (Authorization: Bearer <secret>). This closes
// the trust-on-first-use window so an unpaired instance can't be claimed by
// whoever reaches this endpoint first. With no secret set, first-pair stays TOFU.
export async function POST(req: Request) {
  ...
  try {
    ...
    if (existing) {
      ...
      // First pairing. If an operator secret is set, require it here too.
      const secret = process.env.YGGDRASIL_LINK_SECRET?.trim();
      if (secret && !(await verifyLink(req))) {
        return NextResponse.json({ error: "bootstrap-auth-required" }, { status: 401 });
      }

          ...
          {R.referrals.length > 0 && (
            <div className="mt-5 border-t border-white/8 pt-4">
              ...
              <div className="mt-2 space-y-1.5">
                {R.referrals.slice(0, 4).map((r) => <div key={r.label} className="flex justify-between text-sm"><span className="text-cream/70">{r.label}</span><span className="text-cream/50">{r.count}</span></div>)}
              </div>
            </div>
          )}
        </Reveal>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-white/5">
                {R.byService.length === 0 ? <tr><td colSpan={5} className="px-5 py-6 text-center text-cream/45">No services booked.</td></tr> :
                  R.byService.map((s) => (
                    <tr key={s.name}>
                      <td className="px-5 py-2.5 text-cream/85">{s.name}</td>

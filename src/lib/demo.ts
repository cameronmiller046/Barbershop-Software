...
// The original flagship showcase shop. It used to host the demo accounts and
// their seeded history; the demo now lives on its own DEMO_SLUG tenant so
// reseeds and demo activity never touch this (or any other) real store.
const SHOWCASE_SLUG = "professional-barbershop";
...
/**
 * Get the dedicated demo tenant, creating it on first run. Creation clones the
 * showcase shop's catalog + reviews so the public demo page looks finished,
 * then performs the one-time migration OFF the showcase: purge the demo
 * artifacts the old seeder left there (appointments, clients, time entries) so
 * the real store starts clean and demo reseeds never touch it again.
 */
async function ensureDemoTenant(prisma: PrismaClient) {
  const existing = await prisma.tenant.findUnique({ where: { slug: DEMO_SLUG } });
  if (existing) return existing;

  const showcase = await prisma.tenant.findUnique({ where: { slug: SHOWCASE_SLUG } });
  const tenant = await prisma.tenant.create({
    data: {
      slug: DEMO_SLUG, name: "Demo Store", storeNumber: 999, isDemo: true,
      status: "ACTIVE", plan: "ENTERPRISE", monthlyGoalCents: DEMO_GOAL_CENTS,
      tagline: showcase?.tagline ?? "Where every cut is a masterpiece.",
      description: showcase?.description ?? "The Chair's interactive demo shop — explore booking, the chair-side portal, and reports freely.",
      email: "hello@demo-store.test", phone: "(555) 010-0100", address: "100 Demo Lane, Springfield, IL",
    },
  });

  // Catalog: clone the showcase services (imagery dropped — those may be real
  // uploads) or fall back to the standard templates.
  const src = showcase
    ? await prisma.service.findMany({ where: { tenantId: showcase.id }, orderBy: { sortOrder: "asc" }, select: { name: true, description: true, durationMin: true, priceCents: true, active: true } })
    : [];
  const svcRows = src.length ? src : SERVICE_TEMPLATES;
  await prisma.service.createMany({ data: svcRows.map((s, i) => ({ tenantId: tenant.id, ...s, sortOrder: i })) });

  if (showcase) {
    // Social proof for the public demo page.
    const reviews = await prisma.review.findMany({
      where: { tenantId: showcase.id }, select: { authorName: true, rating: true, body: true, published: true, createdAt: true },
    });
    if (reviews.length) await prisma.review.createMany({ data: reviews.map((r) => ({ ...r, tenantId: tenant.id })) });

    // One-time cleanup: everything below was generated by the old demo seeder
    // (which deleted + recreated it on every reseed, so nothing real lives here).
    await prisma.appointment.deleteMany({ where: { tenantId: showcase.id } });
    await prisma.client.deleteMany({ where: { tenantId: showcase.id } });
    await prisma.timeEntry.deleteMany({ where: { tenantId: showcase.id } });
  }
  return tenant;
}
...
export async function clearDemoData(prisma: PrismaClient) {
  // 1) Delete every store except the demo store and the showcase (cascades
  //    their users + data).
  await prisma.tenant.deleteMany({ where: { slug: { notIn: [DEMO_SLUG, SHOWCASE_SLUG] } } });
...
export async function ensureDemoData(prisma: PrismaClient) {
  const flagship = await ensureDemoTenant(prisma);
...
export async function ensureFlagshipStaff(prisma: PrismaClient) {
  const tenant = await ensureDemoTenant(prisma);
  ...
  // Refresh just these two staff's working hours (idempotent). No tenant filter:
  // this also clears hours left behind on the showcase from before the demo
  // moved to its own store.
  await prisma.workingHour.deleteMany({ where: { barberId: { in: [manager.id, barber.id] } } });

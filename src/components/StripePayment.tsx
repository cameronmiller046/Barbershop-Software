...
export function StripePayment({
  ...
  return (
    <Elements stripe={stripePromise} options={{ clientSecret, appearance }}>
      <InnerForm mode={mode} planLabel={planLabel} price={price} tenantId={tenantId} />
    </Elements>
  );
}

function InnerForm({
  mode, planLabel, price, tenantId,
}: {
  mode: "payment" | "setup";
  planLabel: string;
  price: string;
  ...
  async function submit(e: React.FormEvent) {
    ...
    const return_url = `${window.location.origin}/signup/success?tenant=${tenantId}`;
    const result =
      mode === "setup"
  ...
  return (
    <form onSubmit={submit} className="glass space-y-5 rounded-2xl p-6 sm:p-7">
      <div>
        <h1 className="font-display text-2xl text-cream">Payment details</h1>
      ...
      <button type="submit" disabled={!stripe || loading} className="btn-gold w-full disabled:opacity-50">
        {loading ? "Processing…" : mode === "setup" ? "Start free trial" : `Pay ${price}`}
      </button>
